...
class FirestoreDB:
    ...
    def search_media(
        self, query_text, media_type=None, year=None, category=None, limit=1000
    ):
        """
        Search for media records by text, year, category, etc.

        Args:
            query_text (str): Text to search for in multiple fields
        ...
        # Normalize the query text for consistent matching
        query_text_lower = query_text.lower().strip()
        logger.info(f"Searching for media with query: '{query_text_lower}'")
        ...
        for collection in collections:
            ...
            # Execute query - increase limit to ensure we find all possible matches
            # We'll filter further client-side
            docs = list(query.limit(1000).stream())
            logger.info(f"Found {len(docs)} documents in {collection} before text filtering")

            # Filter by query text manually (since Firestore doesn't support full-text search)
            # Check multiple fields that might contain relevant text
            for doc in docs:
                data = doc.to_dict()
                
                # Create a list of fields to search in, with their values converted to lowercase strings
                search_fields = [
                    str(data.get("session_name", "")).lower(),
                    str(data.get("category", "")).lower(),
                    str(data.get("description", "")).lower(),
                    str(data.get("title", "")).lower(),
                    str(data.get("file_name", "")).lower(),
                    str(data.get("gcs_path", "")).lower(),
                    str(data.get("original_path", "")).lower(),
                    str(data.get("year", "")).lower(),
                ]
                
                # Check if any of the fields contain the query text
                if any(query_text_lower in field for field in search_fields):
                    # Add collection name and document ID to the data
                    data["_collection"] = collection
                    data["_id"] = doc.id
                    results.append(data)
                    logger.debug(f"Matched document: {doc.id} in collection {collection}")

        logger.info(f"Total search matches: {len(results)}")
        
        # Sort results - first by exact title matches, then by creation date (newest first)
        def sort_key(item):
            title = str(item.get("title", "")).lower()
            session_name = str(item.get("session_name", "")).lower()
            
            # Check for exact title match - highest priority
            exact_title_match = title == query_text_lower or session_name == query_text_lower
            
            # Check for title starts with query - second priority
            title_starts_with = title.startswith(query_text_lower) or session_name.startswith(query_text_lower)
            
            # Get created date for time-based sorting
            ...
            # Return tuple for sorting: (exact match, title starts with, -timestamp)
            # The negative timestamp sorts newest first
            return (not exact_title_match, not title_starts_with, -created_timestamp)
            
        sorted_results = sorted(results, key=sort_key)
        
        # Return results, limited to specified limit
