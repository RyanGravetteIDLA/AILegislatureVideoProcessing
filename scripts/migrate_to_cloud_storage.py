...
def migrate_to_gcs(
    ...
    stats = {
        ...
        "error": 0,
        "types": {
            t: {"total": 0, "success": 0, "skipped": 0, "error": 0} for t in media_types
        },
    }
    ...
    for media_type in media_types:
        ...
        if limit and len(files) > limit:
            logger.info(
                f"Limiting to {limit} {media_type} files (from {len(files)} total)"
            )
            files = files[:limit]
        ...
        logger.info(
            f"Migrating {len(files)} {media_type} files to GCS bucket: {bucket_name}"
        )

        # Use tqdm for progress reporting
        with tqdm(
            total=len(files), desc=f"Uploading {media_type}", unit="file"
        ) as progress:
            for i, local_path in enumerate(files):
                ...
                if dry_run:
                    logger.info(
                        f"[DRY RUN] Would upload: {local_path} -> gs://{bucket_name}/{remote_path}"
                    )
                ...
                if blob.exists() and not force:
                    logger.info(
                        f"Skipping (already exists): gs://{bucket_name}/{remote_path}"
                    )
                    stats["types"][media_type]["skipped"] += 1
                    stats["skipped"] += 1
                    progress.update(1)
                ...
                if (i + 1) % batch_size == 0 or i == len(files) - 1:
                    logger.info(
                        f"Progress: {i + 1}/{len(files)} {media_type} files processed"
                    )

        logger.info(
            f"Completed {media_type} migration: {stats['types'][media_type]['success']} succeeded, "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate media files to Google Cloud Storage"
    )
    ...
    # Behavior options
    parser.add_argument(
        "--public", action="store_true", help="Make files publicly accessible"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--force", action="store_true", help="Upload even if file already exists in GCS"
    )
    parser.add_argument(
