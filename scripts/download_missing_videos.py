...
def get_existing_downloads(output_dir, year, category):
    ...
    meeting_pattern = os.path.join(output_dir, year, category, "*_Legislative Session Day *")
    ...
    for meeting_dir in meeting_dirs:
        ...
        date_match = re.search(r'(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d+),\s+\d{4}', dir_name)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download missing videos from Idaho Legislature")
    parser.add_argument('--year', default="2025", 
                       help="Year of the meetings (default: 2025)")
    parser.add_argument('--category', default="House Chambers",
    ...
    existing_downloads = get_existing_downloads(args.output_dir, args.year, args.category)
    print(f"Found {len(existing_downloads)} already downloaded dates: {sorted(existing_downloads)}")
    ...
    for i, meeting in enumerate(missing_meetings, 1):
        ...
        if len(date_parts) >= 2:
            target_date = f"{date_parts[0]} {date_parts[1].rstrip(',')}"  # e.g., "January 8"
            
            print(f"\nDownloading missing meeting {i}/{len(missing_meetings)}: {date}")
            success = downloader.download_specific_meeting(args.year, args.category, target_date)
