...
class AudioTranscriber:
    ...
    def __init__(self, api_key=None, model_name="gemini-2.0-flash"):
        ...
        self.segment_length_ms = 9 * 60 * 1000  # 9 minutes in milliseconds (under Gemini's 10 min limit)
    ...
    def split_audio(self, audio_path):
        ...
        num_segments = (len(audio) // self.segment_length_ms) + (1 if len(audio) % self.segment_length_ms > 0 else 0)
    ...
    def transcribe_segment(self, segment_path):
        ...
        try:
            ...
            response = self.model.generate_content([prompt, {"mime_type": "audio/mpeg", "data": audio_data}])
...
def process_directory(api_key, directory, recursive=False, model_name="gemini-2.0-flash"):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Transcribe audio files using Google's Gemini API")
    parser.add_argument('--api-key', help="Google API key with Gemini access (optional if stored in keychain)")
    ...
    # Directory processor
    dir_parser = subparsers.add_parser('directory', help='Process all audio files in a directory')
    dir_parser.add_argument('directory', help="Directory containing audio files")
    dir_parser.add_argument('--recursive', '-r', action='store_true', help="Process subdirectories")
    ...
    batch_parser = subparsers.add_parser('batch', help='Process all meetings in a year/category')
    batch_parser.add_argument('--output-dir', default="data/downloads", help="Base output directory")
    batch_parser.add_argument('--year', default="2025", help="Year of meetings to process")
    batch_parser.add_argument('--category', default="House Chambers", help="Category of meetings to process")
    ...
    if args.command == 'directory':
        process_directory(api_key, args.directory, args.recursive, model_name=args.model)
        ...
        meeting_dirs = [f.path for f in os.scandir(meetings_path) if f.is_dir() and "Legislative Session Day" in f.name]
