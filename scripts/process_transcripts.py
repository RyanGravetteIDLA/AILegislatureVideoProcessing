...
log_file = os.path.join(
    "data",
    "logs",
    f'process_transcripts_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log',
)
...
def run_full_process(upload_limit=None, retry_errors=False):
    ...
    try:
        ...
        # Step 2: Upload transcripts to Google Cloud Storage
        logger.info("Step 2: Uploading transcripts to Google Cloud Storage...")
        ...
        upload_stats = upload_media_files(
            transcript_files, media_type="transcript", skip_existing=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scan-only",
        action="store_true",
        help="Only scan for transcripts, don't upload",
    )
    parser.add_argument(
        "--upload-limit", type=int, help="Limit the number of files to upload"
    )
    parser.add_argument(
        "--retry-errors", action="store_true", help="Retry previously failed uploads"
    )
    args = parser.parse_args()

    start_time = time.time()
    logger.info(
        f"Starting transcript processing at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    try:
        if args.scan_only:
            ...
            success = run_full_process(
                upload_limit=args.upload_limit, retry_errors=args.retry_errors
            )
            if not success:
