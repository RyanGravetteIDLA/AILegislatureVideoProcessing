...
def convert_existing_videos(videos_dir, audio_format="mp3"):
    ...
    if not shutil.which("ffmpeg"):
        logger.error(
            "ffmpeg not found. Please install ffmpeg to convert videos to audio."
        )
    ...
    downloader = IdahoLegislatureDownloader(
        output_dir=videos_dir, convert_to_audio=True, audio_format=audio_format
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert existing videos to audio format"
    )
    parser.add_argument(
        "--videos-dir",
        "-d",
