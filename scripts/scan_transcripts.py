...
def scan_transcripts():
            ...
            for session_dir in category_dir.glob('*'):
                ...
                for transcript_file in audio_dir.glob('*_transcription.txt'):
                    ...
                    try:
                        ...
                        if not match:
                            logger.warning(f"Unexpected transcript filename format: {file_name}")
                            stats['skipped'] += 1
                            continue
                        ...
                        if transcript:
                            ...
                            stats['new' if transcript.created_at == transcript.updated_at else 'updated'] += 1
                        else:
                            stats['skipped'] += 1
    ...
    logger.info(f"Scan completed. Found: {stats['found']}, New: {stats['new']}, "
...
def generate_report():
    ...
    for i, transcript in enumerate(sorted(transcripts, key=lambda t: t.last_modified or datetime.datetime.min, reverse=True)[:10]):
        status = "✓" if transcript.uploaded else "⨯"
        print(f"{i+1}. [{status}] {transcript.year}/{transcript.category}/{transcript.session_name}/{transcript.file_name}")
