                ...
                if os.path.exists(audio_dir):
                    ...
                    if transcribe and transcriber:
                        ...
                        for audio_file in audio_files:
                            ...
                            transcription_path = (
                                os.path.splitext(audio_file)[0] + "_transcription.txt"
                            )
                            ...
                            try:
                                ...
                                print(
                                    f"Transcription completed for {os.path.basename(audio_file)}"
                                )
                                transcribed_count += 1
                            except Exception as e:
...
def main():
    ...
    if not category:
        ...
        default_category = (
            "House Chambers"
            if "House Chambers" in available_categories
    ...
    target_day = args.day
    if not target_day:
        use_specific_day = get_user_input(
            "Process a specific day only?", options=["y", "n"], default="n"
        )
    ...
    if transcribe and not args.yes:
        ...
        if not api_key:
            ...
            store_key = get_user_input(
                "Do you want to store an API key now?", options=["y", "n"], default="y"
            )
    ...
    if transcribe and not args.yes:
        ...
        confirm_model = get_user_input(
            f"Use {model_name} for transcription?", options=["y", "n"], default="y"
        )
    ...
    if target_day:
        ...
        downloader = IdahoLegislatureDownloader(
            output_dir=args.output_dir, convert_to_audio=True, audio_format="mp3"
        )
                    ...
                    try:
                        ...
                        if os.path.exists(category_dir):
                            for dirname in os.listdir(category_dir):
                                if target_day in dirname and os.path.isdir(
                                    os.path.join(category_dir, dirname)
                                ):
                                    date_dirs.append(
                                        os.path.join(category_dir, dirname)
                                    )
                            ...
                            if os.path.exists(audio_dir):
                                ...
                                for audio_file in audio_files:
                                    ...
                                    if (
                                        ...
                                        and not args.process_existing
                                    ):
                                        print(
                                            f"Skipping transcription - already exists"
                                        )
                                        continue
