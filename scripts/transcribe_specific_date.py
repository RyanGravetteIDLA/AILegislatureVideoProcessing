...
def main():
    ...
    parser = argparse.ArgumentParser(description="Transcribe audio for a specific date from Idaho Legislature")
    ...
    parser.add_argument('--year', default="2025", help="Year of the meeting (default: 2025)")
    parser.add_argument('--category', default="House Chambers", 
                       help="Category of the meeting (default: House Chambers)")
    parser.add_argument('--date', required=True, 
                       help="Date in format 'Month Day' (e.g., 'January 8')")
    ...
    meeting_dir = find_meeting_directory(args.output_dir, args.year, args.category, args.date)
    
    if not meeting_dir:
        print(f"No meeting found for {args.date}, {args.year} in {args.category}")
        print(f"Make sure the meeting has been downloaded first using download_specific_date.py")
        sys.exit(1)
