#!/usr/bin/env python3
"""
Daily script to scan for new files and upload them to Google Cloud Storage.
...
from scripts.upload_media_to_cloud import find_media_files, upload_media_files

# Configure logging
log_dir = os.path.join("data", "logs")
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(
    log_dir, f'daily_upload_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
)
...
def daily_upload(
    recent_only=False, days=1, rate_limit=1, skip_existing=True, batch_size=5
):
    """
    Process daily uploads to Google Cloud Storage.
    ...
    try:
        ...
        for media_type in media_types:
            ...
            if recent_only and days > 0:
                ...
                files = recent_files
                logger.info(
                    f"Found {len(files)} {media_type} files modified in the last {days} days"
                )
        ...
        logger.info(
            f"Daily upload completed in {int(minutes)} minutes, {int(seconds)} seconds"
        )
        ...
        for media_type in media_types:
            ...
            logger.info(
                f"  {media_type.capitalize()}: {media_stats['success']}/{media_stats['total']} uploaded ({success_rate:.1f}%), {media_stats['skipped']} skipped, {media_stats['error']} errors"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Daily script to upload files to Google Cloud Storage"
    )
