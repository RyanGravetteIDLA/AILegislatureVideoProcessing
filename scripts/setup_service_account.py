...
service_account_file = os.path.join(
    credentials_dir, "legislativevideoreviewswithai-80ed70b021b5.json"
)
...
def wait_for_downloaded_key():
    ...
    print(
        "(Looking for files matching *firebase*adminsdk*legislativevideoreviewswithai*.json in Downloads directory)"
    )
    ...
    while time.time() - start_time < max_wait_time:
        for file in os.listdir(downloads_dir):
            if (
                file.endswith(".json")
                and "firebase" in file.lower()
                and "adminsdk" in file.lower()
                and "legislativevideoreviewswithai" in file.lower()
            ):
...
def verify_service_account_key(file_path):
    ...
    try:
        ...
        if actual_email != expected_email and not actual_email.startswith(
            "firebase-adminsdk"
        ):
...
def test_authentication():
    ...
    try:
        ...
        try:
            ...
            print(
                f"Successfully accessed Firestore collections: {', '.join(col.id for col in collections[:5])}"
            )
        except Exception as e:
            print(f"Could access Firestore but not list collections: {e}")
            print(
                "This is normal if your service account doesn't have sufficient permissions."
            )
...
def main():
    ...
    print(
        "This script will help you set up the Firebase Admin SDK service account key."
    )
    ...
    if choice in ("y", "yes"):
        webbrowser.open(
            "https://console.firebase.google.com/project/legislativevideoreviewswithai/settings/serviceaccounts/adminsdk"
        )
    ...
    if not key_path:
        ...
        key_path = input(
            "Please enter the full path to the downloaded service account key: "
        )
    ...
    if auth_success:
        ...
        print(
            "This may be because the service account doesn't have sufficient permissions."
        )
