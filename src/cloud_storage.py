...
Main module for cloud storage operations in the Idaho Legislature Media Portal.
"""
...
class GoogleCloudStorage:
    ...
    def upload_file(
        self, local_path, remote_path=None, make_public=False, content_type=None
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"Uploading {local_path} to gs://{self.bucket_name}/{remote_path} ({file_size/1024/1024:.2f} MB)"
            )
    ...
    def download_file(self, remote_path, local_path):
        ...
        try:
            ...
            # Download the file
            logger.info(
                f"Downloading gs://{self.bucket_name}/{remote_path} to {local_path}"
            )
    ...
    def get_signed_url(self, remote_path, expiration=3600):
        ...
        try:
            ...
            logger.info(
                f"Generated signed URL for {remote_path} (expires in {expiration} seconds)"
            )
            return url
...
def get_default_gcs_client():
    ...
    if not credentials_path:
        # Check for both possible credential file locations
        possible_paths = [
            os.path.join(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bucket",
        default=os.environ.get("GCS_BUCKET_NAME", "idaho-legislature-media"),
    ...
    upload_parser.add_argument(
        "--remote-path", help="Remote file path (default: basename of local path)"
    )
    upload_parser.add_argument(
        "--public", action="store_true", help="Make the file publicly accessible"
    )
    ...
    list_parser.add_argument(
        "--delimiter", help='Delimiter (e.g., "/" for directory-like listing)'
    )
    ...
    url_parser.add_argument(
        "--expiration", type=int, default=3600, help="URL expiration time in seconds"
    )
