...
app.add_middleware(
    CORSMiddleware,
    allow_origins=['https://legislativevideoreviewswithai.web.app', 'https://legislativevideoreviewswithai.firebaseapp.com', 'http://localhost:5173', 'http://localhost:4173'],  # In production, specify exact origins
...
def firestore_to_model(doc_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    base_data = {
        ...
        "date": format_date(doc_data.get('last_modified') or doc_data.get('created_at'))
    }
    ...
    return {
        **base_data,
        "url": base_url,
        "duration": doc_data.get('duration', "00:00:00")  # Placeholder or actual duration if available
    }
...
def get_videos(
    ...
    try:
        if search:
            ...
            docs = db.search_media(search, media_type='video', year=year, category=category)
        else:
...
def get_audio_files(
    ...
    try:
        if search:
            # Use search function if search term provided
            docs = db.search_media(search, media_type='audio', year=year, category=category)
        else:
...
def get_transcripts(
    ...
    try:
        if search:
            # Use search function if search term provided
            docs = db.search_media(search, media_type='transcript', year=year, category=category)
        else:
...
def health_check():
    ...
    return {"status": "healthy", "timestamp": datetime.now().isoformat(), "database": "firestore"}
