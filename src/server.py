...
def run_api(port):
    ...
    if not os.path.exists(
        os.path.join(os.path.dirname(__file__), "transcript_db_sqlite.py")
    ):
        try:
            src_path = os.path.join(os.path.dirname(__file__), "transcript_db.py")
            dst_path = os.path.join(
                os.path.dirname(__file__), "transcript_db_sqlite.py"
            )
            if os.path.exists(src_path):
                ...
                logger.info(
                    f"Backed up SQLite implementation to transcript_db_sqlite.py"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the Idaho Legislature Media backend services"
    )
    parser.add_argument(
        "--api-port",
        type=int,
    ...
    parser.add_argument(
        "--file-port",
        type=int,
        default=int(os.getenv("FILE_PORT", 5001)),
    ...
    try:
        if args.api_only:
            ...
            logger.info(
                f"Starting both API server (port {args.api_port}) and file server (port {args.file_port})..."
            )
            ...
            file_process = multiprocessing.Process(
                target=run_file_server, args=(args.file_port,)
            )
