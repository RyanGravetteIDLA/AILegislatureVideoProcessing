...
class IdahoLegislatureDownloader:
    ...
    def __init__(
        ...
        if self.convert_to_audio:
            ...
            if not self.ffmpeg_available:
                self.logger.warning(
                    "ffmpeg not found. Audio conversion will be skipped."
                )
    ...
    def download_file(self, url, output_path):
        ...
        try:
            ...
            self.logger.info(
                f"Successfully downloaded: {os.path.basename(output_path)}"
            )
            return True
    ...
    def get_available_options(self, soup):
        ...
        try:
            ...
            if category_select:
                ...
                available_categories = [
                    cat for cat in available_categories if "Please Select" not in cat
                ]
    ...
    def get_meeting_results(self, year, category):
        ...
        try:
            ...
            if jsessionid:
                search_url = (
                    f"{self.base_url}/ShowCommitteeOrMedia.do;jsessionid={jsessionid}"
                )
            else:
                search_url = f"{self.base_url}/ShowCommitteeOrMedia.do"

            self.logger.info(
                f"Sending request to {search_url} with year={year}, category={category}"
            )
            ...
            self.logger.error(
                f"Error fetching meeting results for {year}/{category}: {e}"
            )
            return None
    ...
    def extract_meeting_links(self, html_content):
        ...
        try:
            ...
            with open(
                os.path.join(debug_dir, "meeting_results.html"), "w", encoding="utf-8"
            ) as f:
            ...
            for row in rows:
                try:
                    ...
                    download_link = download_cell.find(
                        "a", string="Download Audio/Video"
                    )
                    ...
                    if download_link and download_link.get("href"):
                        ...
                        self.logger.info(
                            f"Found meeting: {meeting_date} - {meeting_title}"
                        )
            ...
            if not meeting_links:
                ...
                for table in tables:
                    ...
                    if (
                        "Date" in header_text
                        or "Committee" in header_text
                        or "Meeting" in header_text
                    ):
                        result_table = table
                        break
                ...
                for row in rows:
                    try:
                        ...
                        meeting_date = (
                            cells[0].text.strip() if len(cells) > 0 else "Unknown Date"
                        )
                        ...
                        for link in links:
                            if link and link.get("href"):
                                ...
                                if (
                                    ...
                                    or "audio" in link_text.lower()
                                    or "video" in link_text.lower()
                                ):
                                    ...
                                    if not download_url.startswith("http"):
                                        download_url = urllib.parse.urljoin(
                                            "https://lso.legislature.idaho.gov",
                                            download_url,
                                        )
                                    ...
                                    self.logger.info(
                                        f"Found meeting with download link: {meeting_date} - {meeting_title}"
                                    )
                        ...
                        self.logger.warning(
                            f"Error extracting meeting link from row: {e}"
                        )
    ...
    def extract_media_urls(self, meeting_url, media_id=None, date=None, title=None):
        ...
        if meeting_url.lower().endswith(".mp4"):
            ...
            media_links.append({"url": meeting_url, "text": "Direct MP4 Link"})
            return media_links
        ...
        if media_id:
            ...
            if direct_url:
                media_links.append({"url": direct_url, "text": "Direct MP4 Link"})
                return media_links
            ...
            if year_match:
                ...
                if date_match:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            self.logger.info(
                                f"Found working insession.idaho.gov URL: {pattern_url}"
                            )
                            media_links.append(
                                {
                                    "url": pattern_url,
                    ...
                    for var_url in variations:
                        try:
                            ...
                            if response.status_code == 200:
                                self.logger.info(
                                    f"Found working variation URL: {var_url}"
                                )
                                media_links.append(
                                    {"url": var_url, "text": "Variation MP4 Link"}
                                )
                                return media_links
                        except Exception as e:
            ...
            for url in sample_urls:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        media_links.append(
                            {"url": url, "text": "Chamber-based MP4 Link"}
                        )
                        return media_links
                except Exception as e:
            ...
            if year_chamber_match:
                ...
                if date_match:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            self.logger.info(
                                f"Found working date-based URL: {pattern_url}"
                            )
                            media_links.append(
                                {"url": pattern_url, "text": "Date-based MP4 Link"}
                            )
                            return media_links
                    except Exception as e:
                        self.logger.debug(f"Error checking URL {pattern_url}: {e}")
        ...
        try:
            ...
            with open(
                os.path.join(debug_dir, "meeting_page.html"), "w", encoding="utf-8"
            ) as f:
            ...
            for link in soup.find_all("a"):
                ...
                if href and ".mp4" in href.lower():
                    ...
                    media_links.append(
                        {"url": href, "text": link.text.strip() or "MP4 Link"}
                    )
            ...
            for video in soup.find_all("video"):
                for source in video.find_all("source"):
                    ...
                    if src and ".mp4" in src.lower():
                        ...
                        media_links.append({"url": src, "text": "Video Source"})
    ...
    def download_specific_meeting(self, year, category, target_date):
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(target_meetings)} meetings for date: {target_date}"
            )

            # Process each meeting for this date
            for meeting in target_meetings:
                # Create a safe directory name
                safe_title = self.create_safe_dirname(
                ...
                category_dir = os.path.join(
                    year_dir, self.create_safe_dirname(category)
                )
                ...
                if (
                    "download" in url_to_use.lower()
                    or "audio" in url_to_use.lower()
                    or "video" in url_to_use.lower()
                ):
                    ...
                    try:
                        ...
                        if success:
                            self.logger.info(
                                f"Successfully downloaded direct file: {filename}"
                            )
                            # Save direct download info
                            media_urls = [
                                {"url": url_to_use, "text": "Direct Download Link"}
                            ]
                            media_info_path = os.path.join(
                                meeting_dir, "media_urls.json"
                            )
                ...
                if not direct_download:
                    media_urls = self.extract_media_urls(
                        url_to_use, media_id, date, title
                    )
                    ...
                    for idx, media in enumerate(media_urls):
                        base_filename = os.path.basename(
                            urllib.parse.urlparse(media["url"]).path
                        )
                        ...
                        self.logger.info(
                            f"Downloading {idx+1}/{len(media_urls)}: {filename}"
                        )
                        ...
                        if success:
                            ...
                            if self.convert_to_audio and self.ffmpeg_available:
                                ...
                                if audio_path:
                                    file_info["audio_filename"] = os.path.basename(
                                        audio_path
                                    )
    ...
    def get_all_meetings(self, year, category):
        ...
        try:
            ...
            if year not in available_years:
                self.logger.error(
                    f"Year {year} not available. Available years: {available_years}"
                )
                return []
            ...
            if category not in available_categories:
                self.logger.error(
                    f"Category {category} not available. Available categories: {available_categories}"
                )
                return []

            self.logger.info(
                f"Fetching meetings for Year: {year}, Category: {category}"
            )
    ...
    def convert_video_to_audio(self, video_path):
        ...
        try:
            ...
            audio_path = os.path.join(
                audio_dir, f"{base_name_no_ext}.{self.audio_format}"
            )
            ...
            cmd = [
                ...
                "-vn",  # Skip video
                "-acodec",
                "libmp3lame" if self.audio_format == "mp3" else "copy",
    ...
    def download_year_category(self, year, category, limit=None):
        ...
        for i, meeting in enumerate(meetings, 1):
            ...
            if len(date_parts) >= 2:
                ...
                self.logger.warning(
                    f"Skipping meeting with invalid date format: {date}"
                )

        self.logger.info(
            f"Completed {year}, {category}. Successfully downloaded {successful_downloads}/{len(meetings)} videos."
        )
