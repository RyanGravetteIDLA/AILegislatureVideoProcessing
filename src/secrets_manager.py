...
class SecretsManager:
    ...
    def store_secret(secret_type, username, value, sensitive=True):
        ...
        try:
            ...
            logger.error(f"Error storing secret {username} in {secret_type} keychain: {e}")
            return False
    ...
    def get_secret(secret_type, username, prompt=None, env_var=None, allow_empty=False):
        ...
        try:
            ...
            logger.warning(f"Error retrieving secret {username} from {secret_type} keychain: {e}")
        ...
        if env_var:
            ...
            if value:
                logger.debug(f"Retrieved secret {username} from environment variable {full_env_var}")
                return value
        
        # If automatic options failed and prompt is provided, ask interactively
        if prompt:
            logger.info(f"Secret {username} not found in keychain or environment, prompting user")
            if "password" in username.lower() or "key" in username.lower() or "token" in username.lower():
        ...
        logger.warning(f"Secret {username} not found and no interactive prompt provided")
        return None
    ...
    def delete_secret(secret_type, username):
        ...
        try:
            ...
            logger.error(f"Error deleting secret {username} from {secret_type} keychain: {e}")
            return False
    ...
    def get_github_credentials():
        ...
        description = SecretsManager.get_secret(
            'github',
            'description',
        ...
        visibility = SecretsManager.get_secret(
            'github',
            'visibility',
    ...
    def get_cloud_storage_settings():
        ...
        for key, prompt_text, env_var, default in [
            ('use_cloud_storage', "Enable cloud storage? (true/false): ", "USE_CLOUD_STORAGE", "false"),
            ('cloud_storage_public', "Make files publicly accessible? (true/false): ", "CLOUD_STORAGE_PUBLIC", "false"),
            ('prefer_cloud_storage', "Prefer cloud over local storage? (true/false): ", "PREFER_CLOUD_STORAGE", "false")
        ]:
            value = SecretsManager.get_secret(
                'cloud_storage',
                env_var,
        ...
        if not json_content:
            ...
            if path and os.path.exists(path):
                try:
                    ...
                    SecretsManager.store_service_account_content(service_type, json_content)
                except Exception as e:
...
if __name__ == "__main__":
    ...
    test_parser.add_argument('--api-keys', action='store_true', help='Test API key retrieval')
    test_parser.add_argument('--github', action='store_true', help='Test GitHub credentials retrieval')
    test_parser.add_argument('--cloud', action='store_true', help='Test cloud storage settings retrieval')
    test_parser.add_argument('--service-accounts', action='store_true', help='Test service account retrieval')
    
    # Migrate command
    migrate_parser = subparsers.add_parser('migrate', help='Migrate existing secrets to the new format')
    ...
    if args.command == 'test':
        ...
        if args.github:
            ...
            if github_creds:
                ...
                print(f"Token: {github_creds['token'][:4]}...{github_creds['token'][-4:]}")
        ...
        try:
            from scripts.manage_api_keys import get_api_key, KEYCHAIN_SERVICE, KEYCHAIN_USERNAME
        ...
        for service_type, path in SERVICE_ACCOUNT_PATHS.items():
            if os.path.exists(path):
                try:
                    ...
                    if SecretsManager.store_service_account_content(service_type, content):
